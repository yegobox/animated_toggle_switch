    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        bool Function(double) isPositionOverIndicator = (dx) =>
            position - max(indicatorSize.width / 2 + dif, 24.0) <= dx &&
            dx <= (position + max(indicatorSize.width / 2 + dif, 24.0));
        ...
        return MouseRegion(
          ...
          onHover: (event) {
            if (hovering ==
                (hovering = isPositionOverIndicator(event.localPosition.dx)))
              return;
            setState(() {});
          },
            ...
            child: TweenAnimationBuilder(
              ...
              builder: (c, color, child) => Container(
                ...
                foregroundDecoration: BoxDecoration(
                  border: Border.all(
                      color: color as Color, width: widget.borderWidth),
